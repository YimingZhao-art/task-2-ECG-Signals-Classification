...
1. Install the requirements
   > pip install -r requirements.txt
   >
2. Run the main.py, this will run the whole process from raw_data to final_prediction
   > python main.py --have_extracted=False
   >
...
The feature extraction uses manually extracted features and last layers of neural network as features.
...
In the implementation of the [repo1](https://github.com/ImaGoodFella/aml_projects/tree/main/project2https://), it lacks the statistics of those features, therefore, the authors concatenated those features as well, which showed improvements compared to the [repo1](https://github.com/ImaGoodFella/aml_projects/tree/main/project2https://). Hence, even though the [repo2](https://github.com/mchami02/ECG-Signal-Classification-Task) only achieved a score about 0.82, it really mentioned important features.

#### Neural Network

This is the part which consumes most of the time in the projects.

1. ENCASE: extract the final layer with 4 features (different from the refered [repo1](https://github.com/ImaGoodFella/aml_projects/tree/main/project2https://))
2. Goodfellow: extract the final layer with 64 features

### Feature Process

1. Abnormal values, including: `nan`, `inf` and `-inf`. The authors impute `nan` with mean and   delete the column with infinity.
2. Feature selection: ~2700 features originally. The authors tested ~2700, 500, 300, which showed that 500 and 2700 are both better. When considering time complexity, 500 may be preferred. But for the final submission, it uses 2700 features.

### Model Prediction

Similar to Project1, the authors still use some of base models and ensemble them. However, there is the main problems of [repo1](https://github.com/ImaGoodFella/aml_projects/tree/main/project2https://). The [repo1](https://github.com/ImaGoodFella/aml_projects/tree/main/project2https://) still uses Stacking instead of MajorityVoting. Accoding to the authors' testing, MajorityVoting (soft) shows a large improvement than Stacking regarding both time (1/5 time) and performance (0.05 higher).

Since the authors uses MajorityVoting (soft), only base models with `predict_proba` can be used. This time, the authors uses:
...
The score after the model name is the validation score, which is used to weight the model in the Voting. To be noted, Gradientboosting is excluded since it is too slow.

The weight is calculated by score with the formula:

$w_i = \frac{e^{\frac{s_i - \min(s)}{\delta}}}{\sum_{j} e^{\frac{s_j - \min(s)}{\delta}}}$

## Reference

[Repo1](https://github.com/ImaGoodFella/aml_projects/tree/main/project2https://)

[Repo2](https://github.com/mchami02/ECG-Signal-Classification-Taskhttps://)
