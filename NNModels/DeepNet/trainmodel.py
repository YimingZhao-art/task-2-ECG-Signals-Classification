...
def train_fold(
    ...
    model = DeepNet(in_channels=18000, n_classes=4)
    ...
    np.savetxt(
        root + "Data/features/" + f"fella_training_features{current_fold}.txt",
        train_features,
        delimiter=",",
    )
    np.savetxt(
        root + "Data/features/" + f"fella_test_features{current_fold}.txt",
        test_features,
        delimiter=",",
    )
